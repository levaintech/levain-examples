...
router.get('/create-wallet-multichain-evm-safe', async (req, res) => {
  try {
    ...
    for (let i = 0; i < networksCaip2Ids.length; i++) {
      ...
      const wallet = await levainGraph.createWallet({
        ...
        type: WalletType.EvmContractSafe,
    ...
    res.status(200).json({
      message:
        'Successfully created a Safe wallet using your keys. Please go to https://app.levain.tech/ to manage your Safe wallet.',
    });
  } catch (error) {
    console.log(error);
